        ...
        if ($result->num_rows > 0) {
            ...
            if ($resultCountPositif->num_rows > 0) {
                while ($rowCountPositif = $resultCountPositif->fetch_assoc()) {
                    ...
                    //untuk menampilkan nilai perhitungan pertama positif
        ...
        if ($result->num_rows > 0) {
            ...
            if ($resultCountNegatif->num_rows > 0) {
                while ($rowCountNegatif = $resultCountNegatif->fetch_assoc()) {
                    ...
                    //untuk menampilkan nilai perhitungan pertama negatif
                                ...
                                <td>
                                    ...
                                    if ($rowSelectDokumenDua ["kelas_dokumen"] == 0) {
                                        if (($positif >= $negatif)||($negatif == 0)) {
                                            echo "Positif";
                                            $sqlInsertHasil = "INSERT INTO tb_hasil VALUES(null,'" . $rowSelectDokumenDua ["deskripsi_dokumen"] . "','" .$positif. "','" .$negatif. "','Positif')";
                                            $conn->query($sqlInsertHasil);


                                        } else if (($positif <= $negatif)||($positif == 0)) {
                                            echo "Negatif";
                                            $sqlInsertHasil = "INSERT INTO tb_hasil VALUES(null,'" . $rowSelectDokumenDua ["deskripsi_dokumen"] . "','" .$positif. "','" .$negatif. "','Negatif')";
                                            $conn->query($sqlInsertHasil);
                                        }

                                        $sqlDeleteDokumen = "DELETE FROM tb_dokumen WHERE kelas_dokumen = 0";
                                        $conn->query($sqlDeleteDokumen);
