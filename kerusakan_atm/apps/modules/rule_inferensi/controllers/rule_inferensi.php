...
class rule_inferensi extends MX_Controller {
    ...
    public function index() {
        ...
        $this->template->load_view('contents/index',array(
                'kerusakan'=>$this->rule_inferensidb->get_drop_array('kerusakan','id_kerusakan','kerusakan'),
    ...
    public function getdatatables(){
        $this->datatables->select('id_kerusakan_gejala,kode_p,kerusakan,kode_g,gejala,datetime,id_kerusakan,kode_p,id_gejala,')
                        ->from('04-view-inferensi');
        echo $this->datatables->generate();
    }
    public function getdatatablesx(){
        $this->datatables->select('id_kerusakan_gejala,id_kerusakan,id_gejala,datetime,')
                        ->from('rule_inferensi');
        echo $this->datatables->generate();
    }
    ...
    public function get($id_kerusakan_gejala=null){
        if($id_kerusakan_gejala!==null){
            echo json_encode($this->rule_inferensidb->get_one($id_kerusakan_gejala));
        }
    }

    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id_kerusakan_gejala')){
            $this->rule_inferensidb->update($this->input->post('id_kerusakan_gejala'));
          }else{
