...
class Wizard extends MX_Controller
{
	...
	public function index() {
		...
		$this->template->load_view('wizard', ['data' => $this->wizardb->get_gejala(),]);
	}
	...
	function setup($id = null) {
		...
		$this->template->set_layout('default');
		$this->template->add_js('var baseurl="' . base_url() . 'wizard/";', 'embed');
		$this->template->load_view('setup', ['data' => $this->wizardb->get_gejala(), 'default' => $this->wizardb->get_all_default($id),]);
	}
	...
	function checklist() {
		...
		$this->template->load_view('checklist', ['data' => $this->wizardb->get_gejala(),]);
	}

	function get_default_val($id1, $id2) {
		$val = $this->kerusakandb->get_default_val($id2, $id2);
		...
		if (($this->input->post('id_gejala')) !== null) {
			...
			if ($data !== false) {
				...
				foreach ($data as $key => $value) {
					# code...
					// echo $value;
					$datax[] = ['kd_proses' => $i, 'id_gejala' => $value, 'densitas' => $this->densitas($value),];
					$i++;
				}
	...
	function e404($msg = null) {
		...
		$this->template->load_view('e404', ['error' => true, 'msg' => $pesan,]);
	}
	...
	function proses_inferensi() {
		...
		if ($input == null):
			...
			$this->template->load_view('data', ['data' => $this->mesin_inferensi($input),]);
		endif;
	}

	function proses_inferensi_print() {
		$this->template->set_title('Wizard');
		$this->template->set_layout('default');
		$this->template->add_js('var baseurl="' . base_url() . 'wizard/";', 'embed');
		$input = $this->input->post('id_gejala');
		if ($input == null):
			...
			$this->template->load_view('dataPrint', ['dataPrint' => $this->mesin_inferensi($input),]);
		endif;
	}
	...
	function mesin_inferensi($input = null) {
		...
		if ($input != null):
			$wizard = $input;
			$kerusakan = $this->wizardb->get_kerusakan();
			$i = 0;
			foreach ($kerusakan as $key => $value) {

				$rule = $this->wizardb->get_rule($value['id_kerusakan']);
				foreach ($rule as $k => $v) {
	...
	function check_array() {
		...
		if ($wizard != null):
			...
			foreach ($kerusakan as $key => $value) {
				# code...
				echo "<h1>(" . $value['kode'] . ") " . $value['kerusakan'] . "</h1>";
				echo "<br>";
				$rule = $this->wizardb->get_rule($value['id_kerusakan']);
				foreach ($rule as $k => $v) {
					# code...
	...
	function proses_wizard() {
		...
		$this->template->set_layout('default');
		$this->template->add_js('var baseurl="' . base_url() . 'wizard/";', 'embed');
		$this->template->load_view('result', ['kepastian' => $this->dempster(), 'dempster' => true,]);
