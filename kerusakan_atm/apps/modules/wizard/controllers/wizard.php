...
class Wizard extends MX_Controller {
	...
	public function index(){
		$this->template->set_title('Kerusakan Mesin');
	...
	function proses_inti($m1,$m2){
		// variabel $m1 adalah variabel awal, bisa jadi variabel gabungan beberapa kerusakan 
		// variabel $m2 adalah variabel gejala kerusakan berikutnya
		//definisi mteta
	...
	function proses_inferensi_print(){
		...
		if($input==null):
			...
			$this->template->load_view('dataPrint',array(
				'dataPrint'=>$this->mesin_inferensi($input),
				'result'=>$this->prepare_to_save(),
			));
		endif;
	}
				...
				*/
				...
				if(($asli==$union) && ($beda==null) && ($selisih==null)){
					...
					$data[$i]['proses']=true;
						...
						$data[$i]['id_kerusakan']=$value['id_kerusakan'];
						...
						//Hasil proses semua kerusakan yang berkaitan dengan gejala
